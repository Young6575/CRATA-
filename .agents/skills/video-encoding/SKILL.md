...
- 화자별 색상은 사용하지 않고 흰색 자막으로 통일한다. 글자 크기, 굵기, 외곽선, 하단 여백은 대시보드의 저장된 자막 스타일 기본값을 따른다.
...
9. 자막 미리보기: 검토 완료된 전사/ASS로 30~60초 미리보기 클립을 생성한다.
10. 미리보기 검수: 자막 크기, 위치, 하단 여백, 줄 수, 화자 색상, 얼굴/자료 화면 가림 여부를 확인할 수 있게 파일 경로를 남긴다.
11. 승인 대기: `video_status.json`을 `waiting_preview_review`로 갱신하고, 사용자가 확인하기 전에는 최종 인코딩을 멈춘다.
12. 승인 후 최종 처리: 사용자가 미리보기를 승인한 경우에만 전체 자막 하드코딩과 최종 인코딩을 진행한다.
13. 완료 후 추가 편집이 필요한 결과물은 `처리관리/video_edit_queue.json`에 추가한다.
