...
- 작업 요청을 받으면 먼저 새 작업 폴더를 만들고 요청 영상 파일을 그 폴더 안으로 복사한다.
- 작업 폴더명과 영상 파일명, 전사·자막·검토 파일명은 전사 완료 후 전사록 내용을 분석한 공통 이름으로 확정한다. 전사 전에는 `_staging_YYYYMMDD_HHMMSS_원본명` 같은 임시 이름을 쓴다.
- 전사록, 검토 리포트, ASS/SRT, 미리보기, 하드코딩 결과, 최종 인코딩 파일은 모두 해당 작업 폴더 안에 저장한다. 원본 소스 폴더에는 산출물을 쓰지 않는다.
...
2. 내부 스크립트의 prepare 명령으로 새 작업 폴더를 만들고 요청 영상 파일을 복사한다.
   ```text
