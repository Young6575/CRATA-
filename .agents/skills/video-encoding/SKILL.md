...
- 작업 폴더명과 영상 파일명, 전사·자막·검토 파일명은 전사 완료 후 전사록 전체의 반복 핵심 용어, 검사명, 역량명을 분석해 정한 짧은 주제형 공통 이름으로 확정한다. 자막 한 줄이나 문장형 발화를 그대로 파일명으로 쓰지 않는다. 전사 전에는 `_staging_YYYYMMDD_HHMMSS_원본명` 같은 임시 이름을 쓴다.
...
7. 전사 완료 직후 내부 스크립트가 전사록 전체의 반복 핵심 용어, 검사명, 역량명을 분석해 짧은 주제형 제목을 정하고 작업 폴더명, 영상 파일명, 원본 전사 파일명을 공통 stem으로 확정한다. 자막 한 줄이나 문장형 발화를 그대로 파일명으로 쓰지 않는다. 이후 단계는 `video_status.json.workspace_path`의 새 경로를 `--base-dir`로 사용한다.
